...
def exact_rank_accuracy(y_true, y_pred):
    ...
    y_pred_rounded = np.clip(y_pred_rounded, 1, 5)  # Ensure predictions are between 1-5
    # Count matches
...
try:
    ...
    y_pred_lr_rounded = np.clip(y_pred_lr_rounded, 1, 5)  # Ensure predictions are between 1-5
    ...
    lr_breakdown = get_prediction_breakdown(y_test, y_pred_lr_rounded)
    
    print(f"Prediction breakdown:")
    print(f"  Exact matches: {lr_breakdown['exact']}")
    print(f"  Off by 1 rank: {lr_breakdown['off_by_one']}")
    print(f"  Off by 2 ranks: {lr_breakdown['off_by_two']}")
    print(f"  Off by >2 ranks: {lr_breakdown['off_by_more']}")
    ...
    # Calculate counts for each type of match
    rf_breakdown = get_prediction_breakdown(y_test, y_pred_rf)
    
    print(f"Prediction breakdown:")
    print(f"  Exact matches: {rf_breakdown['exact']}")
    print(f"  Off by 1 rank: {rf_breakdown['off_by_one']}")
    print(f"  Off by 2 ranks: {rf_breakdown['off_by_two']}")
    print(f"  Off by >2 ranks: {rf_breakdown['off_by_more']}")
    ...
    # Calculate counts for each type of match
    gb_breakdown = get_prediction_breakdown(y_test, y_pred_gb)
    
    print(f"Prediction breakdown:")
    print(f"  Exact matches: {gb_breakdown['exact']}")
    print(f"  Off by 1 rank: {gb_breakdown['off_by_one']}")
    print(f"  Off by 2 ranks: {gb_breakdown['off_by_two']}")
    print(f"  Off by >2 ranks: {gb_breakdown['off_by_more']}")
    ...
    models = ["Linear Regression", "Random Forest", "Gradient Boosting", "MLP Neural Network"]
    exact_accuracies = [acc_lr_exact, acc_rf_exact, acc_gb_exact, acc_mlp_exact]
    flexible_accuracies = [acc_lr_flex, acc_rf_flex, acc_gb_flex, acc_mlp_flex]
